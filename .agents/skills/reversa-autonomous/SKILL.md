...
2. **Handoff automático.** Os skills dos agentes podem terminar sugerindo `continuar`, `loop` ou `clear`. Em modo autônomo, o orquestrador aciona internamente `loop` sem esperar o usuário, respeitando a árvore de unit principal descrita no item seguinte.
3. **Writer avança uma árvore por loop.** Antes do primeiro arquivo de cada unit principal, valide o checkpoint e persista um loop conforme o contrato do Writer: `enabled: true`, `mode: "loop"`, `scope: "unit_tree"`, `root_unit`, `target_orders` finito e seus contadores. O mesmo loop deve concluir os arquivos da unit principal e de todas as subunits descendentes. Ao concluir a árvore, desabilite o loop e persista o checkpoint antes de tocar a próxima unit principal. O orquestrador pode então iniciar um novo loop, sempre para uma única `root_unit`; nunca agrupe árvores, reutilize alvos ou atravesse para globais. Depois de todas as árvores, gere globais separadamente, um arquivo por avanço interno. Essa renovação não exige intervenção humana, mas permanece delimitada, verificável e auditável no estado.
...
7. **Checkpoints continuam obrigatórios.** Salve `state.json` após cada agente, após cada iteração do loop do Writer e ao concluir cada árvore de unit principal, seguindo `checkpoint-guide.md`. O modo autônomo não dispensa a retomabilidade.
